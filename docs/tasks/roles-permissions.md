...
`evaluate(Proposal, WorldView, RoleProfile) -> RolePermission`; allowed targets, duties and hard denials.
...
- [ ] **T02 — Implement the primary behaviour.**
  - **Acceptance:** Evaluate role allowlists, denied actions, target predicates, duties and active cooperation grants from immutable views.
...
- [ ] **T03 — Enforce failures and invariants.**
  - **Acceptance:** Deny unknown roles and forbidden violence regardless of tendencies; prevent an ordinary refusal contradicting a grant without bypassing hard safety.
  - **Tests:** Unknown role denied, peaceful role cannot attack, missing workshop/material prerequisites. Assert typed rejection/fallback and absence of unintended effects, not merely that an exception occurred.
