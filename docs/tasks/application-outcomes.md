...
`accept(ControllerEvent) -> Result[OutcomeReceipt]`; verify event identity, lifecycle, target and plausible execution; request domain effects.
...
- **Integration gate:** [contracts.core](contracts-core.md); [adapters.persistence](adapters-persistence.md); [application.effects](application-effects.md)
...
- [ ] **T02 — Implement the primary behaviour.**
  - **Acceptance:** Validate controller event identity, ordering, timeline, active command, target and plausible accepted spatial evidence before requesting domain effects.
...
- [ ] **T03 — Enforce failures and invariants.**
  - **Acceptance:** Reconcile duplicates and completed-before-cancelled effects; reject fabricated arrival or success without required interaction verification.
  - **Tests:** Forged arrival, old timeline, duplicate completion, success after cancellation and wrong target. Assert typed rejection/fallback and absence of unintended effects, not merely that an exception occurred.
