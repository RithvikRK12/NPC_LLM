...
`eligible(DialogueEvent, Lifecycle, VerifiedOutcomes) -> DialogueDisposition`; gate expression, commitment and outcome lines.
...
- **Integration gate:** [contracts.core](contracts-core.md); [dialogue.compose](dialogue-compose.md)
...
- [ ] **T02 — Implement the primary behaviour.**
  - **Acceptance:** Evaluate expression, commitment and outcome display conditions using lifecycle state and matching verified outcome IDs.
...
- [ ] **T03 — Enforce failures and invariants.**
  - **Acceptance:** Suppress premature success, wrong recipients and repeated display IDs; interruption must update or cancel commitments without rewriting past effects.
  - **Tests:** No outcome line without matching committed event; interrupted commitment; wrong recipient and duplicate display ID. Assert typed rejection/fallback and absence of unintended effects, not merely that an exception occurred.
