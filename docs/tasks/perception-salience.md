...
`rank(FilteredSignals, BudgetPolicy) -> RankedSignals`; assign reproducible priority and category budgets.
...
- **Integration gate:** [contracts.core](contracts-core.md); [perception.relevance](perception-relevance.md)
...
- [ ] **T02 — Implement the primary behaviour.**
  - **Acceptance:** Rank permitted signals reproducibly using priority bands, configurable weights and per-category count/token budgets.
...
- [ ] **T03 — Enforce failures and invariants.**
  - **Acceptance:** Use deterministic tie-breaking; fail boundedly when mandatory evidence alone exceeds budget instead of silently dropping it.
  - **Tests:** Stable ordering, background flood, token/count limits and ties. Mandatory facts exceeding budget cause a bounded failure, not silent loss. Assert typed rejection/fallback and absence of unintended effects, not merely that an exception occurred.
