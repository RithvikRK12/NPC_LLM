...
`submit(InteractionEvent) -> Result[InteractionReceipt]`; deduplicate, validate timeline and route dialogue or explicit game requests.
...
- **Integration gate:** [contracts.core](contracts-core.md); [adapters.persistence](adapters-persistence.md); [domain.world.eligibility](domain-world-eligibility.md)
...
- [ ] **T02 — Implement the primary behaviour.**
  - **Acceptance:** Persist one interaction receipt and decision-work request using the client event ID and canonical payload digest; return the existing receipt on an identical retry.
...
- [ ] **T03 — Enforce failures and invariants.**
  - **Acceptance:** Reject changed payloads under reused IDs and old timelines before dispatch; route explicit game requests through the same eligibility boundary.
  - **Tests:** Duplicate retry, changed payload, stale timeline, unknown target and ineligible interaction. Assert typed rejection/fallback and absence of unintended effects, not merely that an exception occurred.
