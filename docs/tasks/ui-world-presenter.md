...
Render authoritative inventory, quest, refusal/recovery and save/restart state.
...
- **Integration gate:** [contracts.core](contracts-core.md); [gateway.command_client](gateway-command-client.md)
...
- [x] **B01 — Verify reusable baseline.** Saved position/world loading and explicit restart UI exist; controller/recovery/timeline presentation remains.
  ...
  - **Tests/evidence:** [frontend/godot/scripts/main.gd](../../frontend/godot/scripts/main.gd); backend/tests/test_npc_pipeline.py and backend/tests/test_semantic_memory.py (44-test backend suite passed when this plan was created).
...
- [ ] **T02 — Implement the primary behaviour.**
  - **Acceptance:** Render authoritative world snapshots, quest/refusal/recovery status, inventory and accepted saved position without optimistic rewards.
...
- [ ] **T03 — Enforce failures and invariants.**
  - **Acceptance:** Handle pending/failure/reconnect/restart responses and preserve correct timeline; expose same-NPC offer acceptance without client-defined costs.
  - **Tests:** Reconciliation replaces stale display, restart clears prior timeline, pending UI never invents a reward. Assert typed rejection/fallback and absence of unintended effects, not merely that an exception occurred.
