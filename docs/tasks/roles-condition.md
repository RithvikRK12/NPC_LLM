...
`condition(CurrentState, CandidateState, FuzzyResult, RoleProfile, RolePermission) -> RoleConditionedBehavior`; apply delta and tendency modifiers once.
...
- **Integration gate:** [contracts.core](contracts-core.md); [roles.permissions](roles-permissions.md); [state.extract](state-extract.md); [fuzzy.defuzzify](fuzzy-defuzzify.md)
...
- [ ] **T02 — Implement the primary behaviour.**
  - **Acceptance:** Apply role delta multipliers against the original approved base and bound final absolute state, expression and behaviour tendencies.
...
- [ ] **T03 — Enforce failures and invariants.**
  - **Acceptance:** Preserve signed changes and permission denials; retain original/conditioned values and prevent applying a delta twice.
  - **Tests:** Final absolute caps, sign preservation, separation of internal emotion/expression, no denied action reintroduced. Assert typed rejection/fallback and absence of unintended effects, not merely that an exception occurred.
