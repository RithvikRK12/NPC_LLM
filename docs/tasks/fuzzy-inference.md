...
`infer(Memberships, RuleSet) -> AggregatedOutputs`; weighted antecedents and consequent aggregation.
...
- **Integration gate:** [contracts.core](contracts-core.md); [fuzzy.membership](fuzzy-membership.md)
...
- [ ] **T02 — Implement the primary behaviour.**
  - **Acceptance:** Apply min/all and max/any antecedents, weighted activation, clipped consequents and max aggregation; retain contributing rule IDs.
...
- [ ] **T03 — Enforce failures and invariants.**
  - **Acceptance:** Exclude disabled rules, reject unknown variables or invalid weights and retain simultaneous contributions instead of only a winning label.
  - **Tests:** Min/all, max/any, weight bounds, disabled rules, conflicting contributions and rule IDs. Assert typed rejection/fallback and absence of unintended effects, not merely that an exception occurred.
