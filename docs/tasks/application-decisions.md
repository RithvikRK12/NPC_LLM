...
`async decide(DecisionTrigger) -> DecisionOutcome`; assemble the ordered pipeline and route rejection, approval and command scheduling. Does not calculate domain policy.
...
- **Integration gate:** [contracts.core](contracts-core.md); [perception.compose](perception-compose.md); [reasoning.frame](reasoning-frame.md); [reasoning.prompt](reasoning-prompt.md); [adapters.model](adapters-model.md); [reasoning.proposal_check](reasoning-proposal-check.md); [state.extract](state-extract.md); [fuzzy.defuzzify](fuzzy-defuzzify.md); [roles.condition](roles-condition.md); [control.validate](control-validate.md); [application.approvals](application-approvals.md); [application.commands](application-commands.md); [fsec.mapping](fsec-mapping.md)
...
- [ ] **T02 — Implement the primary behaviour.**
  - **Acceptance:** Coordinate perception, reasoning, proposal checks and the agreed pure control sequence using injected ports; release read transactions before model work.
...
- [ ] **T03 — Enforce failures and invariants.**
  - **Acceptance:** Stop on rejected stages, discard stale proposals, persist approval before durable command planning and distinguish no-action from failed execution.
  - **Tests:** Stage ordering, stop on rejection, no DB transaction during inference, stale approval, interrupted decision. Assert typed rejection/fallback and absence of unintended effects, not merely that an exception occurred.
...
### Coordinator task splits

T02 and T03 are integration-only exceptions to narrow leaf-module scope. Assign these bounded slices independently after T01:

- [ ] **T02a — Perception and reasoning slice.** Acceptance: prepare BasePerception/memory/snapshot and terminate model work outside the read transaction. Tests: missing context, model cancellation and source IDs with fake model/reader. Depends on T01 and those boundary fixtures.
- [ ] **T02b — Pure control slice.** Acceptance: extraction → fuzzy → role → final validation ordering; stop at rejected result. Tests: recorded fixture and stage spies prove order and zero persistence. Depends on T01 and control boundary fixtures.
- [ ] **T03a — Approval and command-planning slice.** Acceptance: preserve approved versus base revisions and durable no-action/fallback status. Tests: stale approval and crash after commit do not double-apply. Depends on T02a/T02b and approval/command ports.
- [ ] **T03b — Cancellation and stale-result slice.** Acceptance: superseded work cannot commit, and retries are bounded by policy. Tests: delayed model result after reset and concurrent NPC decision. Depends on T03a.

Check parent T02/T03 only after the corresponding slices pass. These child checkboxes are work breakdown, not extra whole-module completions.
