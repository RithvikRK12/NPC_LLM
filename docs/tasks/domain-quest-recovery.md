...
`offers(QuestBlocker, WorldView, RecoveryPolicy) -> RecoveryOfferSet`; select authored alternate routes and check that their prerequisites remain attainable.
...
- [ ] **T02 — Implement the primary behaviour.**
  - **Acceptance:** Evaluate authored same-NPC favour/trade offers, attainable prerequisites and verified completion predicates; produce a scoped cooperation-grant effect.
...
- [ ] **T03 — Enforce failures and invariants.**
  - **Acceptance:** Reject circular or unavailable recovery routes, changed accepted terms and claim-only completion; suppress a discretionary refusal with no viable alternative.
  - **Tests:** No circular prerequisites, consumed/missing resources, repeated completion, expired offer, no viable route. Recovery uses authored favour/trade offers from the same NPC; completion restores cooperation for the blocked step. Assert typed rejection/fallback and absence of unintended effects, not merely that an exception occurred.
