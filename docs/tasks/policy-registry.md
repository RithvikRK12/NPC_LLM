...
`load(PolicyVersionSet) -> Result[PolicyBundle]`; validate one immutable bundle for a decision.
...
- [ ] **T02 — Implement the primary behaviour.**
  - **Acceptance:** Load and validate an immutable version bundle for profiles, bounds, fuzzy rules, fallbacks, mappings and recovery route schemas.
...
- [ ] **T03 — Enforce failures and invariants.**
  - **Acceptance:** Reject incompatible bounds, unknown engine actions, invalid memberships and incomplete routes; an in-flight decision retains its loaded versions.
  - **Tests:** Missing/unknown actions, inconsistent bounds, unsafe fallback, invalid memberships, incomplete recovery routes. Assert typed rejection/fallback and absence of unintended effects, not merely that an exception occurred.
