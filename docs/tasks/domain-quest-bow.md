...
`evaluate(BowQuestState, VerifiedGameEvent, QuestPolicy) -> QuestTransition`; own materials, pickup, timed crafting and one-time reward.
...
- **Integration gate:** [contracts.core](contracts-core.md); [domain.quest.recovery](domain-quest-recovery.md); [domain.world.inventory](domain-world-inventory.md)
...
- [x] **B01 — Verify reusable baseline.** The bow quest, timed crafting and exactly-one reward have regression coverage; recovery and typed transitions remain.
  ...
  - **Tests/evidence:** [backend/app/services/quests/bow.py](../../backend/app/services/quests/bow.py); backend/tests/test_npc_pipeline.py and backend/tests/test_semantic_memory.py (44-test backend suite passed when this plan was created).
...
- [ ] **T02 — Implement the primary behaviour.**
  - **Acceptance:** Express existing bow phases, wood/string consumption, pickup and server deadline as pure event-driven transitions with one reward identity.
...
- [ ] **T03 — Enforce failures and invariants.**
  - **Acceptance:** Integrate blocked-step recovery and grant fulfilment; reject repeated rewards and early crafting completion without consuming a grant on a failed attempt.
  - **Tests:** Valid phases, repeated reward, missing materials, early completion, resumed deadline and refusal leaving a viable route. Assert typed rejection/fallback and absence of unintended effects, not merely that an exception occurred.
