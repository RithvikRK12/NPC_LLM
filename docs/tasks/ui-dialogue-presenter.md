# ui.dialogue_presenter

## Scope and contract

Present eligible DialogueEvents once; keep per-NPC histories.
...
- **Integration gate:** [contracts.core](contracts-core.md); [gateway.contract_codec](gateway-contract-codec.md); [dialogue.timing](dialogue-timing.md)
...
- [x] **B01 — Verify reusable baseline.** Per-NPC chat windows, history restoration and clear-chat exist; lifecycle-timed event delivery remains.
  ...
  - **Tests/evidence:** [frontend/godot/scripts/chat_window.gd](../../frontend/godot/scripts/chat_window.gd); backend/tests/test_npc_pipeline.py and backend/tests/test_semantic_memory.py (44-test backend suite passed when this plan was created).
...
- [ ] **T02 — Implement the primary behaviour.**
  - **Acceptance:** Render eligible backend dialogue events once into the appropriate NPC history and retain display/event identity across reconciliation.
...
- [ ] **T03 — Enforce failures and invariants.**
  - **Acceptance:** Hide unverified success and wrong-NPC events; restore histories on resume and discard old timeline callbacks on restart.
  - **Tests:** Wrong NPC, duplicate display ID, suppressed unverified success, interrupted commitment. Assert typed rejection/fallback and absence of unintended effects, not merely that an exception occurred.
