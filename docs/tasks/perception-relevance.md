...
`filter(Signals, DecisionContext, RelevancePolicy) -> FilterResult`; include only useful, permitted evidence with reasons.
...
- **Integration gate:** [contracts.core](contracts-core.md); [perception.normalize](perception-normalize.md)
...
- [ ] **T02 — Implement the primary behaviour.**
  - **Acceptance:** Filter signals against NPC knowledge scope and the current interaction, quest and target; emit inclusion and exclusion reasons.
...
- [ ] **T03 — Enforce failures and invariants.**
  - **Acceptance:** Preserve the current request and known urgent threats while excluding unseen/private facts and unrelated or expired context.
  - **Tests:** Private/unseen facts excluded; current request and urgent visible threats retained. Assert typed rejection/fallback and absence of unintended effects, not merely that an exception occurred.
