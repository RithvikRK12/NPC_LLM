...
`async generate(ModelRequest) -> Result[RawProposal]`; call configured local/compatible provider with cancellation and deadlines.

Source: [Detailed design](../detailed-design.md), 4.2 Perception and reasoning modules; [proposal](../proposal.md). The confirmed detailed-design ordering overrides conflicting proposal examples.
...
- [x] **B01 — Verify reusable baseline.** Provider abstraction, HTTP generation and mock provider exist; target cancellation and typed proposal parity remain.
  ...
  - **Tests/evidence:** [backend/app/services/llm/provider.py](../../backend/app/services/llm/provider.py); backend/tests/test_npc_pipeline.py and backend/tests/test_semantic_memory.py (44-test backend suite passed when this plan was created).
...
- [ ] **T02 — Implement the primary behaviour.**
  - **Acceptance:** Wrap the configured local/compatible model behind an asynchronous provider result with deadlines, cancellation and response-size limits.
...
- [ ] **T03 — Enforce failures and invariants.**
  - **Acceptance:** Map connection, malformed response and cancellation failures to typed provider errors without retrying gameplay effects.
  - **Tests:** Timeout, malformed response, oversized output, cancelled request and provider error; fake provider contract parity. Assert typed rejection/fallback and absence of unintended effects, not merely that an exception occurred.
