...
`approve(ValidationResult, ExpectedRevisions) -> Result[ApprovalReceipt]`; commit internal changes, decision history and a durable command-planning record once.
...
- **Integration gate:** [contracts.core](contracts-core.md); [adapters.persistence](adapters-persistence.md); [control.validate](control-validate.md)
...
- [ ] **T02 — Implement the primary behaviour.**
  - **Acceptance:** Atomically compare relevant revisions and persist internal changes, approval history and durable command-planning work once.
...
- [ ] **T03 — Enforce failures and invariants.**
  - **Acceptance:** Keep outcome-dependent changes pending; retry/crash recovery must not reapply internal state and must expose the new approved revision.
  - **Tests:** CAS conflict, duplicate decision, crash recovery, unchanged outcome-dependent values. Assert typed rejection/fallback and absence of unintended effects, not merely that an exception occurred.
