...
`commit(ApprovedEffect, ExpectedRevisions) -> Result[VerifiedDomainEvent]`; atomically apply ownership/quest/outcome-dependent changes and their durable events.
...
- **Integration gate:** [contracts.core](contracts-core.md); [adapters.persistence](adapters-persistence.md); [domain.world.inventory](domain-world-inventory.md); [domain.quest.bow](domain-quest-bow.md)
...
- [ ] **T02 — Implement the primary behaviour.**
  - **Acceptance:** Commit approved inventory/quest effects, their outcome records and delivery intent in a short transaction with relevant revision checks.
...
- [ ] **T03 — Enforce failures and invariants.**
  - **Acceptance:** Deduplicate effect identities, roll back all owners on failure and atomically couple trade costs with cooperation grants; never announce an uncommitted reward.
  - **Tests:** Rollback, duplicate event, concurrent consumption, stale ownership, crash before/after commit. Assert typed rejection/fallback and absence of unintended effects, not merely that an exception occurred.
