...
`select(EligibleCandidates, UtilityPolicy, ApprovedHistory) -> SelectionResult`; component scoring, switching margin and deterministic tie-break.
...
- **Integration gate:** [contracts.core](contracts-core.md); [fsec.eligibility](fsec-eligibility.md)
...
- [ ] **T02 — Implement the primary behaviour.**
  - **Acceptance:** Score eligible candidates by versioned utility components and apply the documented deterministic action-ID tie-break.
...
- [ ] **T03 — Enforce failures and invariants.**
  - **Acceptance:** Apply switching margins to running actions; an ineligible high-score action cannot win and no eligible candidate returns a declared no-action/fallback.
  - **Tests:** Ineligible high-score action cannot win; stable ties; insufficient advantage does not interrupt. Assert typed rejection/fallback and absence of unintended effects, not merely that an exception occurred.
