...
`compose(BasePerception, RetrievedMemories, RankedSignals) -> PerceptionSnapshot`; freeze the model input with provenance.
...
- **Integration gate:** [contracts.core](contracts-core.md); [perception.salience](perception-salience.md); [memory.retrieve](memory-retrieve.md)
...
- [ ] **T02 — Implement the primary behaviour.**
  - **Acceptance:** Compose one immutable PerceptionSnapshot from BasePerception, selected signals and already retrieved memory, retaining revisions and provenance.
...
- [ ] **T03 — Enforce failures and invariants.**
  - **Acceptance:** Build the memory query before the final snapshot; prevent circular retrieval and prevent later mutation of inputs from changing snapshot content.
  - **Tests:** Input mutation cannot alter snapshot; memory scope retained; no circular memory lookup. Assert typed rejection/fallback and absence of unintended effects, not merely that an exception occurred.
