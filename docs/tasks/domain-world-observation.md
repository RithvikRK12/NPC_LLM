...
`observe(NPCView, WorldView, SensePolicy) -> ObservedSignals`; limit direct/informed knowledge and distinguish reports from accepted facts.
...
- [ ] **T02 — Implement the primary behaviour.**
  - **Acceptance:** Collect direct, informed and internal observations through read-only world and spatial ports; attach stable sources and knowledge scopes.
...
- [ ] **T03 — Enforce failures and invariants.**
  - **Acceptance:** Exclude out-of-range or occluded entities, private state of other NPCs and expired information; expose no global event without a propagation rule.
  - **Tests:** Occlusion, out-of-range signals, private NPC state, expired knowledge and event propagation. Assert typed rejection/fallback and absence of unintended effects, not merely that an exception occurred.
