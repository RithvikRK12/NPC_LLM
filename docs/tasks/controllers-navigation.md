...
`start(target, limits)`, `tick(dt)`, `stop()`; report arrival/blocked/failed through lifecycle.
...
- **Integration gate:** [contracts.core](contracts-core.md); [controllers.lifecycle](controllers-lifecycle.md)
...
- [ ] **T02 — Implement the primary behaviour.**
  - **Acceptance:** Implement bounded navigation start/tick/stop against a fakeable path/physics port and report arrival or failure through lifecycle events.
...
- [ ] **T03 — Enforce failures and invariants.**
  - **Acceptance:** Handle blocked/moving targets, timeouts and stationary capabilities without passing through obstacles or reporting fictitious arrival.
  - **Tests:** Missing/blocked paths, moving targets, bounds and stationary NPC using a fake navigation port; separate real physics scene checks. Assert typed rejection/fallback and absence of unintended effects, not merely that an exception occurred.
