...
`build(Snapshot, ReasoningProfile, DecisionPolicy) -> ReasoningInput`; expose goals and deterministic conflict priorities.
...
- **Integration gate:** [contracts.core](contracts-core.md); [perception.compose](perception-compose.md)
...
- [ ] **T02 — Implement the primary behaviour.**
  - **Acceptance:** Build a bounded decision frame from the snapshot, stable reasoning profile, goals and explicit conflict priorities.
...
- [ ] **T03 — Enforce failures and invariants.**
  - **Acceptance:** Reject missing conflict evidence and keep safety, role rules and current observations above preferences and unverified claims.
  - **Tests:** Safety over cooperation, profile stability, current facts over claims, unknown conflict evidence. Assert typed rejection/fallback and absence of unintended effects, not merely that an exception occurred.
