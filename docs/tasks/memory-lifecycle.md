...
`review(Records, Now, RetentionPolicy) -> LifecyclePlan`; distinguish expiry for retrieval from archival/deletion.
...
- [ ] **T02 — Implement the primary behaviour.**
  - **Acceptance:** Evaluate expiry, archive and correction visibility under explicit per-type policy and return a mutation plan rather than writing directly.
...
- [ ] **T03 — Enforce failures and invariants.**
  - **Acceptance:** Separate salience decay from deletion, preserve required provenance and perform no automatic deletion when retention is unspecified.
  - **Tests:** Temporary expiry, archive exclusion, protected provenance and no deletion when policy is unspecified. Assert typed rejection/fallback and absence of unintended effects, not merely that an exception occurred.
