...
`issue(ApprovalReceipt, CommandDraft) -> Result[Command]`; assign ordered controller revision and persist delivery/lifecycle.
...
- **Integration gate:** [contracts.core](contracts-core.md); [adapters.persistence](adapters-persistence.md); [fsec.mapping](fsec-mapping.md); [application.approvals](application-approvals.md)
...
- [ ] **T02 — Implement the primary behaviour.**
  - **Acceptance:** Issue one command per planning identity with an ordered NPC/controller revision, expiry and durable delivery record.
...
- [ ] **T03 — Enforce failures and invariants.**
  - **Acceptance:** Reject stale approval, duplicate terminal transitions and old timelines; redelivery returns the same command rather than issuing a new effect.
  - **Tests:** Repeated issue, stale approval, terminal-state uniqueness, expiry and per-NPC ordering. Assert typed rejection/fallback and absence of unintended effects, not merely that an exception occurred.
