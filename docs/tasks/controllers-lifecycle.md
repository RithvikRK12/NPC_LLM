...
`accept(command)`, `handle(event)`, `cancel(reason)`; only component allowed to change active command state.
...
- [ ] **T02 — Implement the primary behaviour.**
  - **Acceptance:** Implement a table-driven command state machine with acceptance, start, progress and terminal events owned by one active command.
...
- [ ] **T03 — Enforce failures and invariants.**
  - **Acceptance:** Validate interruption/expiry and deduplicate receipt; terminal states are stable and interaction success requires verified evidence.
  - **Tests:** Every transition, duplicate receipt, expiry, interruption, terminal-state stability. Assert typed rejection/fallback and absence of unintended effects, not merely that an exception occurred.
