...
`extract(CurrentState, MergedChanges) -> CandidateState`; relative-to-absolute conversion with canonical clamps and provenance.
...
- **Integration gate:** [contracts.core](contracts-core.md); [state.merge](state-merge.md)
...
- [ ] **T02 — Implement the primary behaviour.**
  - **Acceptance:** Convert signed relative changes to canonical absolute candidates while recording previous value, accepted delta, clamp and source.
...
- [ ] **T03 — Enforce failures and invariants.**
  - **Acceptance:** Preserve the original approved base, keep internal versus outcome timing separate and disallow world-effect fields without mutating ORM state.
  - **Tests:** Signed deltas, exact bounds, no ORM writes, deterministic output and no inventory/quest fields. Assert typed rejection/fallback and absence of unintended effects, not merely that an exception occurred.
