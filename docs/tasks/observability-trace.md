...
`record(StageTrace)` and `measure(MetricSample)`; emit correlated redacted diagnostics.
...
- [ ] **T02 — Implement the primary behaviour.**
  - **Acceptance:** Emit stage traces and metric samples with causal IDs, policy/model versions and bounded structured diagnostics through an injected sink.
...
- [ ] **T03 — Enforce failures and invariants.**
  - **Acceptance:** Redact secrets and excluded private fields; sink failure must not change a pure decision or misrepresent an uncommitted effect.
  - **Tests:** Missing correlation, redaction, bounded payload and sink failure not changing a domain decision. Assert typed rejection/fallback and absence of unintended effects, not merely that an exception occurred.
