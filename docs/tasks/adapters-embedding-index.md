# adapters.embedding_index

## Scope and contract

`embed_documents`, `embed_query`, `search(IndexScope, vector, limit)`; isolate Ollama/Nomic and FAISS details.
...
- [x] **B01 — Verify reusable baseline.** Real Nomic embeddings, FAISS HNSW, reindexing and transaction-aware cache reuse exist; timeline and target-port support remain.
  ...
  - **Tests/evidence:** [backend/app/services/memory/embedding.py](../../backend/app/services/memory/embedding.py); backend/tests/test_npc_pipeline.py and backend/tests/test_semantic_memory.py (44-test backend suite passed when this plan was created).
...
- [ ] **T02 — Implement the primary behaviour.**
  - **Acceptance:** Expose existing Nomic document/query embeddings and FAISS ID/score search through namespace-aware ports with timeline-scoped indexes.
...
- [ ] **T03 — Enforce failures and invariants.**
  - **Acceptance:** Preserve finite normalized dimensions and reject mixed models; exercise rollback/reset/eviction rebuild and provider outage without hash fallback.
  - **Tests:** Dimensions/model namespace, finite normalized vectors, reset/rollback invalidation, rebuild and scoped cache reuse. Assert typed rejection/fallback and absence of unintended effects, not merely that an exception occurred.
