...
Submit typed interaction attempt and wait for verified effect.
...
- **Integration gate:** [contracts.core](contracts-core.md); [controllers.lifecycle](controllers-lifecycle.md); [gateway.command_client](gateway-command-client.md)
...
- [ ] **T02 — Implement the primary behaviour.**
  - **Acceptance:** Submit command-linked interaction attempts and wait for backend effect verification before marking the interaction successful.
...
- [ ] **T03 — Enforce failures and invariants.**
  - **Acceptance:** Preserve identities over duplicate acknowledgements, target/range failure and interrupted gestures; local animation cannot grant an item.
  - **Tests:** Out-of-range target, rejected transfer, duplicate acknowledgement and interrupted gesture. Assert typed rejection/fallback and absence of unintended effects, not merely that an exception occurred.
