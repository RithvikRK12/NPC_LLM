...
`candidates(ValidatedBehavior, Capabilities, WorldView) -> EligibleCandidates`; intersect engine capability and approved constraints.
...
- **Integration gate:** [contracts.core](contracts-core.md); [control.validate](control-validate.md)
...
- [ ] **T02 — Implement the primary behaviour.**
  - **Acceptance:** Intersect the validated allowed set with fresh target, navigation, cooldown, animation and controller interruptibility evidence.
...
- [ ] **T03 — Enforce failures and invariants.**
  - **Acceptance:** Remove role-denied or unavailable actions before scoring; an active grant excludes ordinary refusal but does not create missing material or a path.
  - **Tests:** No path, missing target, noninterruptible command, cooldown and denied actions. Assert typed rejection/fallback and absence of unintended effects, not merely that an exception occurred.
