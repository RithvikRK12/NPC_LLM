...
`smooth(RoleConditionedBehavior, ApprovedHistory, ControlPolicy, Now) -> ContinuityResult`; rate bounds and emergency override evidence.
...
- **Integration gate:** [contracts.core](contracts-core.md); [roles.condition](roles-condition.md)
...
- [ ] **T02 — Implement the primary behaviour.**
  - **Acceptance:** Smooth the conditioned candidate against prior approved history using elapsed time, configured alpha and rate limits.
...
- [ ] **T03 — Enforce failures and invariants.**
  - **Acceptance:** Use only verified urgent evidence for an override and keep final values inside both role and canonical ranges; rejected history has no influence.
  - **Tests:** Alternating proposals, elapsed time, ordinary versus urgent transitions, no smoothing from rejected history. Assert typed rejection/fallback and absence of unintended effects, not merely that an exception occurred.
