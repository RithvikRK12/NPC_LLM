...
`render(ReasoningInput) -> ModelRequest`; serialize trusted instructions separately from bounded historical/player data.
...
- **Integration gate:** [contracts.core](contracts-core.md); [reasoning.frame](reasoning-frame.md)
...
- [ ] **T02 — Implement the primary behaviour.**
  - **Acceptance:** Render ReasoningInput into versioned model instructions, schema and separately labelled snapshot data within response/context budgets.
...
- [ ] **T03 — Enforce failures and invariants.**
  - **Acceptance:** Keep retrieved instructions as untrusted quoted data; omit credentials and out-of-scope private fields and avoid unclassified history injection.
  - **Tests:** Prompt budgets, injection-like memory remains data, no credentials/private excluded fields. Assert typed rejection/fallback and absence of unintended effects, not merely that an exception occurred.
