...
`propose(VerifiedMemory, InfluencePolicy, AppliedEffectLedger) -> InfluenceProposals`; bounded state/relationship effects with source identity.
...
- **Integration gate:** [contracts.core](contracts-core.md); [memory.classify](memory-classify.md)
...
- [ ] **T02 — Implement the primary behaviour.**
  - **Acceptance:** Produce bounded source-linked influence proposals only from eligible verified memories, with target dimensions and authoritative application timing.
...
- [ ] **T03 — Enforce failures and invariants.**
  - **Acceptance:** Skip already-applied effect identities and distinguish verified helpful outcomes from claims; retrieval itself must never change trust.
  - **Tests:** Repeated retrieval has no side effects, verified help versus unverified claim, same outcome not rewarded twice. Assert typed rejection/fallback and absence of unintended effects, not merely that an exception occurred.
