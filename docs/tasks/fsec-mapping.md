...
`map(SelectionResult, ValidatedBehavior, Capabilities) -> Result[CommandDraft]`; clamp engine parameters and attach dialogue timing.
...
- **Integration gate:** [contracts.core](contracts-core.md); [fsec.utility](fsec-utility.md); [dialogue.compose](dialogue-compose.md)
...
- [ ] **T02 — Implement the primary behaviour.**
  - **Acceptance:** Map a selection to an engine-neutral CommandDraft with bounded movement, animation, target and dialogue-timing parameters.
...
- [ ] **T03 — Enforce failures and invariants.**
  - **Acceptance:** Preserve the validated ConstraintSet, reject unsupported mappings and prevent an outcome line without its required verified event.
  - **Tests:** Negative speed, unsupported animation, missing outcome dependency and constraints never widened. Assert typed rejection/fallback and absence of unintended effects, not merely that an exception occurred.
