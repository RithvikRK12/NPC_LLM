...
`shape(AggregatedOutputs, OutputPolicy) -> BehaviorShapingResult`; centroid tendencies and declared no-activation fallback.
...
- **Integration gate:** [contracts.core](contracts-core.md); [fuzzy.inference](fuzzy-inference.md)
...
- [ ] **T02 — Implement the primary behaviour.**
  - **Acceptance:** Calculate centroid tendencies and return BehaviorShapingResult with configuration versions and rule contribution diagnostics.
...
- [ ] **T03 — Enforce failures and invariants.**
  - **Acceptance:** Use a declared safe no-activation result for a valid empty set; reject invalid numeric configuration and prevent division-by-zero/non-finite output.
  - **Tests:** Zero denominator, bounded outputs, precision tolerance, contribution trace and repeatability. Assert typed rejection/fallback and absence of unintended effects, not merely that an exception occurred.
