...
Map lifecycle and expression to supported visual states; expose capability fallback.
...
- [ ] **T02 — Implement the primary behaviour.**
  - **Acceptance:** Map controller states and permitted expression parameters to available animations or declared visual fallbacks.
...
- [ ] **T03 — Enforce failures and invariants.**
  - **Acceptance:** Clamp playback/blend parameters and keep animation callbacks from completing inventory/quest effects.
  - **Tests:** Missing clips, bounded blend/speed, no animation callback directly grants inventory. Assert typed rejection/fallback and absence of unintended effects, not merely that an exception occurred.
