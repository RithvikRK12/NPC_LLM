...
`load(GameRef) -> WorldView`; `restart(RestartRequest) -> Result[WorldView]`; coordinate save, resume and timeline replacement.
...
- **Integration gate:** [contracts.core](contracts-core.md); [adapters.persistence](adapters-persistence.md)
...
- [x] **B01 — Verify reusable baseline.** Saved world/position and explicit reset exist; timeline-safe invalidation and restart idempotency are not yet implemented.
  ...
  - **Tests/evidence:** [backend/app/api/routes/world.py](../../backend/app/api/routes/world.py); backend/tests/test_npc_pipeline.py and backend/tests/test_semantic_memory.py (44-test backend suite passed when this plan was created).
...
- [ ] **T02 — Implement the primary behaviour.**
  - **Acceptance:** Load the current saved timeline and implement idempotent explicit restart as a new timeline plus initial gameplay state.
...
- [ ] **T03 — Enforce failures and invariants.**
  - **Acceptance:** Invalidate previous-timeline decisions, commands, memory jobs and grants; preserve normal resume and deduplicate a lost restart response.
  - **Tests:** Reopen preserves progress; reset clears playable history; late old-timeline callback cannot mutate new state. Assert typed rejection/fallback and absence of unintended effects, not merely that an exception occurred.
